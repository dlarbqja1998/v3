...
describe('지도 셔틀 출발 카운트다운', () => {
	...
	it('같은 시각에도 두 정류장의 실제 다음 출발을 따로 계산한다', () => {
		...
		expect(getShuttleCountdown(now, 'campus')).toMatchObject({ label: '28분 후', departureTime: '09:30', directionLabel: '조치원역행' });
		expect(getShuttleCountdown(now, 'jochewon-station-back')).toMatchObject({ label: '8분 후', departureTime: '09:10', directionLabel: '학교행' });
	});

	it.each([
		['08:59:00', '1분 후', false],
		['08:59:01', '59초 후', true],
		['08:59:58.500', '2초 후', true],
		['08:59:59.999', '1초 후', true],
		['09:00:00', '30분 후', false]
	...
	it('오송행을 조치원역행으로 잘못 표시하지 않는다', () => {
		expect(getShuttleCountdown(new Date('2026-09-14T17:59:01+09:00'), 'campus')).toMatchObject({ directionLabel: '오송역행', departureTime: '18:00', label: '59초 후' });
	});

	it('막차 이후·토요일에는 다음 날까지의 큰 분 수 대신 오늘 상태를 표시한다', () => {
		expect(getShuttleCountdown(new Date('2026-09-14T20:40:00+09:00'), 'campus')).toMatchObject({ status: 'finished', label: '오늘 운행 종료', secondsLeft: null });
	...
	it.each(['UTC', 'America/Los_Angeles', 'Asia/Seoul'])('%s 기기에서도 한국 날짜·시각으로 계산한다', (timezone) => {
		...
		expect(getShuttleCountdown(new Date('2026-09-14T00:29:01Z'), 'campus').label).toBe('59초 후');
		// UTC에서는 아직 일요일이어도 한국에서는 월요일 첫차를 기다린다.
		expect(getShuttleCountdown(new Date('2026-09-13T15:00:00Z'), 'campus')).toMatchObject({ departureTime: '09:00', label: '540분 후' });
	});
});
