...
import { festivalPreview, formatFestivalPrice, getFestivalBooths, getFestivalTodayEntry, getFestivalBoothForSession } from './festival';

describe('축제 부스 정보', () => {
	it('낮·밤 상세 전환은 같은 동아리의 다른 번호 부스로 연결된다', () => {
		for (const [day, night] of [['day-booth-16', 'night-booth-2'], ['day-booth-10', 'night-booth-4'], ['day-booth-2', 'night-booth-9']]) {
			expect(getFestivalBoothForSession(festivalPreview, day, 'night')?.id).toBe(night);
			expect(getFestivalBoothForSession(festivalPreview, night, 'day')?.id).toBe(day);
		}
		expect(getFestivalBoothForSession(festivalPreview, 'day-booth-4', 'night')).toBeUndefined();
		expect(getFestivalBoothForSession(festivalPreview, 'day-booth-16', 'night', '2026-09-16')).toBeUndefined();
	});
	...
	it('낮부스를 번호순으로 제공하고 홍보 자료가 없는 부스는 비워둔다', () => {
		...
		expect(booths.filter((item) => !item.sourceUrl).every((item) => item.sessions.day?.items.length === 0)).toBe(true);
		expect(booths.every((item) => !item.sessions.night)).toBe(true);
	...
	it('밤 부스는 제공된 번호순으로 표시하고 낮 운영을 임의로 추가하지 않는다', () => {
		...
		expect(booths.every((item) => !item.sessions.day)).toBe(true);
		expect(booths.filter((item) => !item.sourceUrl).every((item) => item.sessions.night?.items.length === 0)).toBe(true);
	});
	it('무료와 가격 미정을 구분한다', () => {
